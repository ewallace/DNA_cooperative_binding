...
plot_fbound <- function(fbound_df, title="fbound vs protein concentration") {
    ggplot(data=fbound_df,aes(x=conc,colour=factor(n_prot))) +
        geom_point(aes(y=fraction_bound)) +
        scale_x_log10() +
...
plot_fbound_fit <- function(fbound_df, title="fbound vs protein concentration",
    ...
    # plot fraction bound AND fitted data with mycoef
    fbound_df$predicted_fbound <- with(erre61pal_df,combined_fit_natscale(conc,n_prot,
    ...
    plot_fbound(fbound_df, newtitle) + 
        geom_line(aes(y=predicted_fbound),linetype="dashed")
    }
